...
if(!(await q('SELECT id FROM usuario LIMIT 1')).length){
 if(production&&(!process.env.ADMIN_PASSWORD||Buffer.byteLength(process.env.ADMIN_PASSWORD)<12||Buffer.byteLength(process.env.ADMIN_PASSWORD)>72))throw Error('Configure ADMIN_PASSWORD com 12 a 72 bytes antes do primeiro deploy.');
 const password=process.env.ADMIN_PASSWORD || randomBytes(12).toString('base64url');
 await q('INSERT INTO usuario(nome,login,senha_hash) VALUES($1,$2,$3)',['Administrador','admin',await bcrypt.hash(password,12)]);
 if(!production)console.log(`Primeiro acesso — usuário: admin | senha: ${password}`);
}
...
app.post('/api/login',async(req,res)=>{
 ...
 const user=(await q('SELECT * FROM usuario WHERE login=$1',[String(req.body.login||'')]))[0];
 ...
 attempts.delete(key);const token=randomBytes(32).toString('hex');await q("DELETE FROM sessao WHERE expira < NOW()");await q("INSERT INTO sessao VALUES($1,$2,NOW()+INTERVAL '8 hours')",[createHash('sha256').update(token).digest('hex'),user.id]);
 res.set('Set-Cookie',`signy=${token}; HttpOnly; SameSite=Strict; Path=/; Max-Age=28800${req.secure?'; Secure':''}`).json({nome:user.nome});
});
app.use('/api',async(req,res,next)=>{try{const user=(await q('SELECT u.id,u.nome FROM sessao s JOIN usuario u ON u.id=s.id_usuario WHERE token=$1 AND expira>NOW()',[sessions(req)]))[0];if(!user)return res.status(401).json({error:'Entre na sua conta para continuar.'});req.user=user;next();}catch(e){next(e);}});
...
app.post('/api/password',async(req,res)=>{const u=(await q('SELECT * FROM usuario WHERE id=$1',[req.user.id]))[0];if(!await bcrypt.compare(String(req.body.atual||''),u.senha_hash))throw Error('Senha atual incorreta.');if(typeof req.body.nova!=='string'||req.body.nova.length<10||req.body.nova.length>72)throw Error('Use uma senha de 10 a 72 caracteres.');await q('UPDATE usuario SET senha_hash=$1 WHERE id=$2',[await bcrypt.hash(req.body.nova,12),u.id]);await q('DELETE FROM sessao WHERE id_usuario=$1 AND token<>$2',[u.id,sessions(req)]);res.json({ok:true});});
...
async function exercises(tx,id,list){if(!Array.isArray(list)||list.length>100)throw Error('Informe uma lista válida de exercícios.');await tx.query('DELETE FROM ficha_exercicio WHERE id_ficha=$1',[id]);for(const item of list){await insert(tx,'ficha_exercicio',{id_ficha:id,id_exercicio:item.id_exercicio,ordem:item.ordem,series:item.series,repeticoes:item.repeticoes,carga_sugerida:item.carga_sugerida===''?null:item.carga_sugerida,observacao:item.observacao||null});}}
